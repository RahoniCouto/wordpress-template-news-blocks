...
function sortPostsByDateAndId(posts = []) {
  return [...posts].sort((firstPost, secondPost) => {
    const firstDate = typeof firstPost?.date === 'string' ? firstPost.date : '';
...
export default function useNewsSectionPosts({
  ...
  const normalizedExcludedPostIds = useMemo(
    () => normalizePostIds(excludePostIds).filter((postId) => postId > 0),
    [excludePostIds],
  );

  const manualPostIds = useMemo(
    () =>
      normalizePostIds(normalizedSlotPostIds).filter((postId) => postId > 0),
  ...
  return useSelect(
    (select) => {
      ...
      const slotSources = ['automatic', 'automatic', 'automatic', 'automatic'];
      ...
      if (selectionMode === 'automatic') {
        const entityRecordsArgs = ['postType', 'post', automaticQuery];

        const queriedPosts = core.getEntityRecords(...entityRecordsArgs) || [];

        automaticPosts = sortPostsByDateAndId(queriedPosts);

        if (
          !core.hasFinishedResolution('getEntityRecords', entityRecordsArgs)
        ) {
          isResolving = true;
        }
