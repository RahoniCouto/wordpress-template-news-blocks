...
function NewsSectionPostEditor({
  ...
  const readingTimeMinutes = Number(post?.meta?._wtn_reading_time_minutes) || 0;

  const readingTime =
    readingTimeMinutes > 0
      ? sprintf(
          /* translators: %d: persisted reading time in minutes. */
          _n(
            '%d min de leitura',
            '%d min de leitura',
            readingTimeMinutes,
    ...
    <article className={`${classPrefix}-card`}>
      ...
      <div className={`${classPrefix}-content`}>
        ...
        <div className={`${classPrefix}-meta`}>
          {formattedDate && (
            <span className={`${classPrefix}-meta-item`}>{formattedDate}</span>
          )}

          {readingTime && (
            <span className={`${classPrefix}-meta-item`}>{readingTime}</span>
          )}
        </div>
      </div>
    </article>
...
export default function Edit({ attributes, setAttributes, clientId }) {
  ...
  const handleCategoryChange = (nextCategoryId) => {
    ...
    const nextSlotPostIds = normalizedSlotPostIds.map((postId, slotIndex) => {
      if (!postId) {
        return 0;
      }
      ...
      return postCategoryIds.includes(normalizedNextCategoryId) ? postId : 0;
    });
  ...
  useEffect(() => {
    ...
    if (arePostIdListsEqual(localResolvedPostIds, computedResolvedPostIds)) {
      return;
    }
  ...
  const defaultViewAllLabel = __('Ver todas', 'wordpress-template-news-blocks');
  ...
  const inspectorControls = (
    <InspectorControls>
      <PanelBody
        title={__('Configuração da seção', 'wordpress-template-news-blocks')}
        initialOpen
      >
        <CategoryPicker
      ...
      <PanelBody
        title={__('Link “Ver todas”', 'wordpress-template-news-blocks')}
        initialOpen={false}
      >
        <TextControl
      ...
      <PanelBody
        title={__('Seleção de matérias', 'wordpress-template-news-blocks')}
        initialOpen
      >
        <RadioControl
          label={__('Modo de seleção', 'wordpress-template-news-blocks')}
          selected={selectionMode}
          options={[
            {
              label: __('Automática', 'wordpress-template-news-blocks'),
              value: 'automatic',
            },
            {
              label: __('Manual', 'wordpress-template-news-blocks'),
              value: 'manual',
            },
          ]}
        ...
        <div className="wtn-blocks-news-section__slot-controls">
          {slotLabels.map((slotLabel, slotIndex) => (
            <EditorialPostSlotControl
              ...
              configuredPostId={normalizedSlotPostIds[slotIndex]}
              resolvedPostId={computedResolvedPostIds[slotIndex]}
              resolvedPost={resolvedPosts[slotIndex]}
              source={slotSources[slotIndex]}
              categoryId={normalizedCategoryId}
              excludePostIds={getSlotExcludedPostIds(slotIndex)}
              onChange={(nextPostId) => {
      ...
      >
        <RadioControl
          label={__('Variação visual', 'wordpress-template-news-blocks')}
          ...
          <>
            <NewsSectionPostEditor
              ...
              onChange={(nextPostOverride) => {
                handlePostOverrideChange(featuredPostId, nextPostOverride);
              }}
              variant="featured"
            ...
            <div className="wtn-blocks-news-section__secondary-list">
              {secondaryPosts.map((post, secondaryIndex) => {
                const postId = secondaryPostIds[secondaryIndex] || 0;
                ...
                return (
                  <NewsSectionPostEditor
                    ...
                    onChange={(nextPostOverride) => {
                      handlePostOverrideChange(postId, nextPostOverride);
                    }}
                    variant="secondary"
                    sectionCategoryId={normalizedCategoryId}
                  />
